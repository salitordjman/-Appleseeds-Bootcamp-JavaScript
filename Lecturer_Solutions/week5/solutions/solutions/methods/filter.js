...
const numbers = [1, 100, 67, -5, 6784, 356, 200];
const greatThan = (arr, number) => arr.filter((num) => num > number);
// console.log(greatThan(numbers, 100));
...
const greaterThanAndLessThan = (arr, number1, number2) => {
  return arr.filter((num) => {
    return num > number1 && num < number2;
  });
};
console.log(greaterThanAndLessThan(numbers, 100, 1000));
...
const filterRatings = (arr, rating) => {
  return arr.filter((movie) => {
    return movie.rating.average > rating;
  });
};
const xx = filterRating(movies, 7);
