...
const raitingnum=(arrRait, raitnum)=>{
  // return arrRait>raitnum
  return arrRait.rating.average>raitnum
}

const raitingcalc =(arr,callback)=>{
  let movier=[];
  for(let elm in arr){
    // if(callback(arr[elm].rating.average, 8)) {movier[elm]=arr[elm]}
    // if(callback(arr[elm], 8)) {movier[elm]=arr[elm]}
    if(callback(arr[elm], 8)) {movier.push(arr[elm])}
  }
 return movier
}
console.log(raitingcalc(movies,raitingnum));